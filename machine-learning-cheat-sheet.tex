...
\begin{description}
\item[$\mathbb{N}$] the set of all neighbours
\item[$\mathbb{P}$] the set of all parents
\end{description}
...
A matrix $\bf{M}$ is
\begin{description}
\item[\stabilo{\textbf{positive (semi-)definite}}] $\iff \forall$ nonzero vector $\bf{a}$, we have $\bf{a}^T \bf{M} \bf{a} >(=) \; 0$
\item[\textbf{symmetric}] $\bf{M}^T = \bf{M}$
\item[singular] if not invertible
\item[\stabilo{invertible}] $\iff$ full rank $\iff$ \pd\ $\iff$ diag \& diag$\neq0$ $\iff$ $\bf{M}^T$ invertible
\end{description}
...
The gradient of the MSE set to zero is called the  \stabilo{normal equation}~:
...
We can use a Directed Acyclic Graph (DAG) to define a \stabilo{joint distribution} of events~: %For example, we can express the relationship between \textit{latent factors} (possible ``causes'') $z_i$ and \textit{observations} (results) $y_i$:
...
\begin{description}
\item[\stabilo{variables to observations}]
...
\item $\bf{K}$ \psd%: $\forall$ nonzero vector $\bf{a}, \bf{a}^T \bf{K} \bf{a} \geq 0$.
\end{itemize}
...
\hrule
\tiny
Rendered \today. Written by Dennis Meier, Merlin Nimier-David and Marc Bourqui.
...
Source code available on~: \href{https://github.com/mbourqui/pcml-cheatsheet}{https://github.com/mbourqui/pcml-cheatsheet}
